...
SELECT mov.title,
       AVG(rts.avg_rating) AS avg_rating,
       GROUP_CONCAT(g.genre) AS genres
FROM   movie AS mov
       INNER JOIN ratings AS rts
               ON mov.id = rts.movie_id
       INNER JOIN genre AS g
               ON mov.id = g.movie_id
WHERE  mov.title LIKE 'The%'
       AND rts.avg_rating > 8
GROUP  BY mov.title;
...
(
    SELECT
        ...
        DENSE_RANK() OVER (PARTITION BY year ORDER BY CAST(REPLACE(REPLACE(COALESCE(worlwide_gross_income, '0'), 'INR', ''), '$', '') AS DECIMAL(10)) DESC) AS movie_rank
    FROM
        movie AS m
    ...
    GROUP BY
        m.id, movie_name, year, worlwide_gross_income
)
SELECT *
FROM movie_summary
WHERE movie_rank <= 5
